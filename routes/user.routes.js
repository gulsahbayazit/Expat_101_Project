...
router.post("/signup", (req, res, next) => {
    const {userName, firstName, lastName, country, occupation, email, password} = req.body
    ...
    if (userName === " ") {
      ...
      User.findOne({ userName }).then((userFromDB) => {
        ...
        res.render("signup.hbs", {message: "Password needs to have at least one Upppercase letter, one number and be 6 characters long at least"})
            return;
            }
        ...
        User.create({ userName: userName, firstName: firstName,lastName: lastName,country: country,occupation: occupation,email: email,
            password: hash  })
            .then((createdUser) => {
